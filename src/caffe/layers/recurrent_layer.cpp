...
void RecurrentLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (layer_name.size()) {
    for (int i = 0; i < net_param.layer_size(); ++i) {
      ...
      std::string orgname = layer->name();
      layer->set_name(layer_name + "_" + layer->name());
      printf("[ZXD][LSTM] set internal layer %d name: %s => %s ...\n",
          i, orgname.c_str(), layer->name().c_str());
  ...
  for (int i = 0; i < unrolled_net_->params().size(); ++i) {
    if (unrolled_net_->param_owners()[i] == -1) {
      ...
      printf("[ZXD][RecurrentLayer] Adding parameter %d: %s, shape %s\n", 
          i, unrolled_net_->param_display_names()[i].c_str(), unrolled_net_->params()[i]->shape_string().c_str());
      this->blobs_.push_back(unrolled_net_->params()[i]);
    }
  }
  
  printf("[ZXD][RecurrentLayer] There are totally %d blobs!\n", this->blobs_.size());
  printf("[ZXD] Press any key to continue...\n");
  // getchar();
...
void RecurrentLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  { int fcnt = 0;
  /*
  printf("[ZXD][RecurrentLayer] #forward %d: x_input_blob_(size=%d, name='x') = %s\n",
      fcnt, x_input_blob_->count(), print_blob_data(x_input_blob_).c_str());
  printf("[ZXD][RecurrentLayer] #forward %d: cont_input_blob_(size=%d, name='cont') = %s\n",
      fcnt, cont_input_blob_->count(), print_blob_data(cont_input_blob_).c_str());
      */
  printf("[ZXD][RecurrentLayer] #forward %d: x_input_blob_(size=%d, name='x')\n",
      fcnt, x_input_blob_->count());
  printf("[ZXD][RecurrentLayer] #forward %d: cont_input_blob_(size=%d, name='cont')\n",
      fcnt, cont_input_blob_->count());
  ...
  for (int i = 0; i < recur_input_blobs_.size(); ++i) {
      /*
      printf("[ZXD][RecurrentLayer] #forward %d: recur_input_blobs_[%d](size=%d, name='%s') = %s\n",
          fcnt, i, recur_input_blobs_[i]->count(), recur_input_names[i].c_str(), print_blob_data(recur_input_blobs_[i]).c_str());
      printf("[ZXD][RecurrentLayer] #forward %d: recur_output_blobs_[%d](size=%d, name='%s') = %s\n",
          fcnt, i, recur_output_blobs_[i]->count(), recur_output_names[i].c_str(), print_blob_data(recur_output_blobs_[i]).c_str());
          */
      printf("[ZXD][RecurrentLayer] #forward %d: recur_input_blobs_[%d](size=%d, name='%s')\n",
          fcnt, i, recur_input_blobs_[i]->count(), recur_input_names[i].c_str());
      printf("[ZXD][RecurrentLayer] #forward %d: recur_output_blobs_[%d](size=%d, name='%s')\n",
          fcnt, i, recur_output_blobs_[i]->count(), recur_output_names[i].c_str());

  }
  printf("[ZXD][RecurrentLayer] Press any key to continue...\n");
  }
