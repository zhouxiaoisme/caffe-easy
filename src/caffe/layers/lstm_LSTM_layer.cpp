...
#include "caffe/layers/lstm_LSTM_layer.hpp"
...
void LSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  printf("[ZXD][LSTMLayer] input_shapes[0].shape = %s\n", tostring(input_shapes[0]).c_str());
  printf("[ZXD][LSTMLayer] input_shapes[1].shape = %s\n", tostring(input_shapes[1]).c_str());
